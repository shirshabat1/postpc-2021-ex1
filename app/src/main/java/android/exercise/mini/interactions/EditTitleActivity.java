...
public class EditTitleActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    {
      v.animate().scaleX((float) 1.35).scaleY((float)1.35).setDuration(500L).
    withEndAction(() -> {
      v.animate().alpha(0.8f).scaleX((float) 0.75).scaleY((float)0.75).setDuration(500L);
    ...
    fabEditDone.setOnClickListener(v ->
    {
      ...
      v.animate().scaleX((float) 1.35).scaleY((float)1.35).setDuration(500L).
              withEndAction(() -> {
                        v.animate().scaleX((float) 0.75).scaleY((float) 0.75).setDuration(500L);
                        v.setVisibility(View.INVISIBLE);
                        fabStartEdit.setVisibility(View.VISIBLE);
              });
      fabStartEdit.setVisibility(View.VISIBLE);
  ...
  public void onBackPressed() {
    ...
    if (this.isEditing)
    {
      editTextTitle.setVisibility(View.INVISIBLE);
        ...
        fabEditDone.animate().scaleX((float) 1.35).scaleY((float)1.35).setDuration(500L).
                withEndAction(() -> {
                  fabEditDone.animate().scaleX((float) 0.75).scaleY((float) 0.75).setDuration(500L);
